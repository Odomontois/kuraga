...
object ExampleGADT:
  given Eq[K2, ExampleGADT] with
    extension [A[_[_]], B[_[_]]](k: ExampleGADT[A])
      def isEq(k2: ExampleGADT[B]) =
        [R] =>
          (eq: GEQ[K2, ExampleGADT, A, B, R]) =>
            k match
              case ExampleGADT.S =>
                k2 match
  ...
  given [TC[_[_[_]]]](using tcInt: TC[LI], tcString: TC[LS]): Has[K2, TC, ExampleGADT] with
    extension [A[_[_]]](gadt: ExampleGADT[A])
      def constraintsFor: TC[A] = gadt match
