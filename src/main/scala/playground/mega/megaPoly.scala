...
type M[F[_[_ <: Result]], A] = F[
  [T <: Result] =>> T match {
    case Result => A
  }
]

type BiA[F[_[_ <: Result | Error]], E, A] = F[
  [T <: Result | Error] =>> T match {
    case Error  => E
    case Result => A
  }
]

type ProA[F[_[_ <: Result | Context]], R, A] = F[
  [T <: Context | Result] =>> T match {
    case Context => R
    case Result  => A
  }
]

type Reap[F[_[_ <: Context | Error | Result]], R, E, A] = F[
  [T <: Context | Error | Result] =>> T match {
    case Context => R
    case Result  => A
    case Error   => E
  }
