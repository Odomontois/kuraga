...
trait Layer1[-P[-i[+_], +o[+_]], +A]:
  ...
  def flatMap[Q[-i[+_], +o[+_]], B](
      f: A => Layer1[Q, B]
  ): Layer1[[i[+_], o[+_]] =>> Q[i, o] & P[i, o] & EvalF[i, o], B] = new {
...
object Layer1:
  ...
  extension [X](self: Ev[X]) def value: X = Layer1.run[EvalF, X](self)(EvalF.EvalInterpreter[EvalF]())
...
object EvalF:
  ...
  final case class K[A, -P[-i[+_], +o[+_]] <: EvalF[i, o], +B](fa: Layer1[P, A], cont: A => Layer1[P, B])
      extends Layer1[P, B]:
    def unpack[X[+_]](p: P[L1[P], X]): X[B] = p.flatMap[A, B](fa, cont)

  def interpret[P[-i[+_], +o[+_]] <: EvalF[i, o], A, B](k: K[A, P, B])(step: P[L1[P], L1[P]]): Layer1[P, B] =
    k.fa.unpack(step) match
      case Done(b)  => k.cont(b)
      case K(fx, f) => fx.flatMap(x => f(x).flatMap(k.cont))
      case ev       => ev.flatMap(k.cont)

  def interpretErr[P[-i[+_], +o[+_]] <: ErrorEvF[E][i, o], A, B, E](k: K[A, P, B])(
      step: P[L1[P], L1[P]]
  ): Layer1[P, B]                                                                                           =
    k.fa.unpack(step) match
      case Done(b)       => k.cont(b)
