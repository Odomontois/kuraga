...
import scala.caps.Read
import scala.caps.SharedCapability
import scala.caps.ExclusiveCapability

opaque type Task = String

class MyState private (private var count: Int) extends ExclusiveCapability
trait Metric {
...
def reportState1(using state: MyState): String ->{state} String = { name =>
  s"$name current value is ${MyState.count}\n"
}
