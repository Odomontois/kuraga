...
trait Apply[F[_]] extends Functor[F]:
  ...
  extension [A, B, C](fa: Eval[F[A]])
    def map2Lz(fb: Eval[F[B]])(f: (A, B) => Eval[C]): Eval[F[C]] =
      for
        fae <- fa
        fbr <- fb
...
trait Monad[F[_]] extends FlatMapTail[F] with Applicative[F]:
  extension [A, B, C](fa: F[A])
    override def map2(fb: F[B])(f: (A, B) => C): F[C] =
      fa.flatMap(a => fb.flatMap(b => pure(f(a, b))))

trait StackSafeMonad[F[_]] extends Monad[F]:
  extension [A, B](a: A)
    override def tailRecM(f: A => F[Either[A, B]]): F[B] =
      f(a).flatMap {
