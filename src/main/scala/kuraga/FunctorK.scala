...
trait FunctorK[U[f[_]]]:
  extension [F[_], G[_]](obj: U[F]) def mapK(f: [A] => F[A] => G[A]): U[G]

trait ApplyK[U[f[_]]] extends FunctorK[U]:
  extension [F[_], G[_], H[_]](left: U[F]) def map2K(right: U[G])(f: [A] => (F[A], G[A]) => H[A]): U[H]
...
trait ApplicativeK[U[f[_]]] extends ApplyK[U] with PureK[U]:
  ...
  extension [F[_], G[_]](obj: U[F])
    override def mapK(f: [A] => F[A] => G[A]): U[G] =
      obj.map2K(pureK[[A] =>> Unit]([A] => () => ()))([A] => (x: F[A], y: Unit) => f(x))
...
trait TraverseK[U[f[_]]] extends FunctorK[U]:
  extension [F[_], G[+_], H[_]](uf: U[F]) def traverseK(f: [A] => F[A] => G[H[A]])(using Applicative[G]): G[U[H]]
