...
object Show:
    ...
    private inline def showTuple[ET <: Tuple, EL <: Tuple](xs: ET): List[String] = inline xs match
        case ht : (h *: t) => inline erasedValue[EL] match 
            case _ : (hl *: tl) => 
                val hl = summonInline[ValueOf[hl]].value
                val sh = summonInline[Show[h]]
                val hs = sh.show(ht.head)
                val head = s"$hl: $hs"
                head :: showTuple[t, tl](ht.tail)

        case _: EmptyTuple => Nil
    
    private inline def productShow[A <: Product](using m: Mirror.ProductOf[A]): Show[A] = new:
        def show(a: A) = showTuple[m.MirroredElemTypes, m.MirroredElemLabels](Tuple.fromProductTyped(a)).mkString("{", ",", "}")
